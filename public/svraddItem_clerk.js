...
app.post('/process/addItem_clerk', (req, res)=>{ //req가 웹브라우저에서 올라온 정보받기, res가 웹브라우저에게 답변할때 정보
    console.log('/process/addItem_clerk 호출됨'+ req)

    // 페이지 로드 시 세션 확인
    checkSession();

    const paramshopID = req.body.shopID; // 보낸걸 각각 뽑기 시작
    const paramItemID = req.body.itemID;
    const paramItemNAME = req.body.itemNAME; // 이것들 가져다가 db 테이블에 보낼꺼임-> 이젠 커넥션이 필요
...
// 아래코드, 커넥션을 얻음
    ...
    const exec = conn.query('insert into itemdb (shopID, itemID, itemNAME) values (?, ?, ?)',
    [paramshopID, paramItemID, paramItemNAME],
    (err, result)=>{
